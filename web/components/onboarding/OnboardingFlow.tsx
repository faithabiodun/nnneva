...
import { stepsFor, type Role } from "@/lib/onboarding";

/**
 * Onboarding: one question at a time, with the reason for asking pinned beside
 * it. The last step writes whatever the role implies — a care profile, a
 * trusted contact, a request to help someone — and the first memories, so the
 * agent's very first run already has context.
 *
 * Which questions get asked comes entirely from the role. There is no shared
 * "and also" section: a supporter never sees a due date field, not even a
 * skippable one, because a skippable question they cannot answer is still a
 * question they have to read and dismiss.
 */
export function OnboardingFlow({ role }: { role: Role }) {
  ...
  function problemWith(index: number): string | null {
    ...
    switch (step.kind) {
      ...
      case "birthdate":
        return birthDateProblem(answers);
      case "supporting":
        return answers.supportingUsername.trim()
          ? null
          : "Enter their username, or skip if they have not joined yet.";
      case "place":
      ...
      case "choice":
        // A choice step with options is a plain single-select bound to one
        // answer; without them it is the clock.
        if (step.field) {
          return answers[step.field] ? null : "Choose one to continue.";
        }
  ...
  function next(skipping = false) {
    ...
    startSaving(async () => {
      try {
        await completeOnboarding(toPayload(answers, role));
      } catch (error) {
          ...
          <div key={i} className="animate-rise">
            ...
            {step.kind === "birthdate" && (
              <DueDateCalendar
                ...
                onChange={(d) => set("birthDate", d)}
                {...birthDateRange()}
              />
            )}

            {step.kind === "supporting" && (
              <div className="max-w-[440px]">
                <label className="block">
                  <span className="mb-1.5 block text-[12px] text-muted-2">
                    Their username
                  </span>
                  <div className="flex items-center gap-2 rounded-[11px] bg-white px-4 shadow-[0_1px_2px_rgba(11,44,34,0.05)] focus-within:ring-2 focus-within:ring-green">
                    <span aria-hidden className="text-[15px] text-faint">
                      @
                    </span>
                    <input
                      value={answers.supportingUsername}
                      onChange={(e) => set("supportingUsername", e.target.value)}
                      autoCapitalize="none"
                      autoCorrect="off"
                      spellCheck={false}
                      placeholder="their-username"
                      className="min-w-0 flex-1 bg-transparent py-3.5 text-[15px] text-ink outline-none"
                    />
                  </div>
                </label>
                <p className="mt-2.5 text-caption text-faint">
                  They will get a request to approve. Nothing is shared until they do.
                </p>
              </div>
            )}
            ...
            {step.kind === "choice" &&
              (step.field ? (
                <Choices
                  options={step.options ?? []}
                  selected={answers[step.field] ? [String(answers[step.field])] : []}
                  onPick={(label) => {
                    const key = step.field!;
                    set(key, answers[key] === label ? "" : label);
                  }}
                />
              ) : (
