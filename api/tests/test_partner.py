...
@pytest.fixture
def contact_id(client, signed_up, db):
    """Her first contact's id. Onboarding creates it; the routes are per-contact."""
...
@pytest.fixture
def invited(client, signed_up, db, contact_id):
    ...
    db.commit()
    return client.post(f"/contacts/{contact_id}/invite", headers=signed_up).json()["access_token"]
...
def test_inviting_produces_a_long_random_token(client, signed_up, contact_id):
    body = client.post(f"/contacts/{contact_id}/invite", headers=signed_up).json()
    assert body["invited"] is True
    assert body["accepted"] is False
    assert len(body["access_token"]) >= 40, "a guessable link is the whole vulnerability"


def test_re_inviting_rotates_and_kills_the_old_link(client, signed_up, contact_id):
    first = client.post(f"/contacts/{contact_id}/invite", headers=signed_up).json()["access_token"]
    second = client.post(f"/contacts/{contact_id}/invite", headers=signed_up).json()["access_token"]
...
def test_revoking_stops_the_link(client, signed_up, invited, contact_id):
    assert client.get(f"/partner/{invited}").status_code == 200
    assert client.delete(f"/contacts/{contact_id}/invite", headers=signed_up).status_code == 204
...
def test_both_sides_see_one_thread(client, signed_up, invited, contact_id):
    client.post(f"/contacts/{contact_id}/messages", headers=signed_up,
                json={"body": "Could you pick up my iron tablets?"})
    client.post(f"/partner/{invited}/messages", json={"body": "Already got them."})

    hers = client.get(f"/contacts/{contact_id}/messages", headers=signed_up).json()
...
def test_opening_the_link_marks_it_accepted(client, signed_up, invited, contact_id):
    assert client.get(f"/contacts/{contact_id}", headers=signed_up).json()["accepted"] is False
    client.get(f"/partner/{invited}")
    assert client.get(f"/contacts/{contact_id}", headers=signed_up).json()["accepted"] is True
...
def test_an_assigned_task_reaches_them_and_can_be_completed(
    client, signed_up, invited, contact_id, her_tasks, db
):
    task_id = her_tasks[0]

    client.post(f"/contacts/{contact_id}/tasks/{task_id}", headers=signed_up)
...
def test_unassigning_takes_it_back(client, signed_up, invited, contact_id, her_tasks):
    client.post(f"/contacts/{contact_id}/tasks/{her_tasks[0]}", headers=signed_up)
    client.delete(f"/tasks/{her_tasks[0]}/assign", headers=signed_up)
    assert client.get(f"/partner/{invited}").json()["tasks"] == []


def test_they_only_ever_see_tasks_she_assigned(client, signed_up, invited, contact_id, her_tasks):
